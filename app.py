...
if menu == "🏠 Inicio":
    ...
    try:
        ...
        for i, fila in df_config.iterrows():
            with col_1 if i < 2 else col_2:
                # Si en el Excel dice "Cerrado", el switch aparecerá encendido
    ...
    with st.spinner("📡 Sincronizando reloj satelital con DB_HORARIOS..."):
        try:
            ...
            df_horarios = df_horarios.dropna(subset=['DÍA', 'BLOQUE_HORARIO']) # Limpieza básica
        except Exception as e:
            st.error("🚨 Falla Crítica: No se encontró la pestaña 'DB_HORARIOS' en la base satelital. Cree la pestaña en su Google Sheets y vuelva a intentarlo.")
            st.stop()
    ...
    # --- 1. LÓGICA DE VISTAS (ADMIN VS DOCENTE) ---
    if st.session_state.rol == "Admin":
    ...
    # --- 3. DIBUJADO DE ALTA COSTURA (HTML/CSS) ---
            ...
            for d in dias_orden:
                if d in matriz.columns:
                    ...
                    if val == "":
                        ...
                        # Si hay una colisión (dos clases en el mismo bloque), se pinta de rojo
                        if "<hr" in val:
                            bg_color = "#ffe6e6"
                            borde_fuerte = "border:2px solid #cc0000;"
                        else:
                            bg_color = "#e6ffe6" if st.session_state.rol != "Admin" or tipo_vista == "🧑‍🏫 Vista por Docente" else "#e6f2ff"
    ...
    # Si hubo colisión, lanzar advertencia
    if "<hr" in html_tabla:
    ...
    # --- 4. MAPA DE CARGA ACADÉMICA (AUDITORÍA ADMIN) ---
    if st.session_state.rol == "Admin":
        st.markdown("<hr>", unsafe_allow_html=True)
        st.markdown("#### 📊 Carga Académica de la Tropa (Horas Semanales por Docente)")
        
        # Filtramos para no contar los descansos ni horas libres
