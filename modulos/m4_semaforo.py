...
def renderizar(*args, **kwargs):
    ...
    st.markdown("""
        <style>
            ...
            .titular-semaforo { font-family: 'Arial Black', sans-serif; font-size: 16px; font-weight: bold; margin-bottom: 10px; }
            .numero-critico-3d { font-size: 85px; font-family: 'Arial Black', sans-serif; font-weight: 900; color: #cc0000; display: inline-block; animation: titilar_critico 1.2s infinite ease-in-out; }
            .numero-alerta-3d { font-size: 85px; font-family: 'Arial Black', sans-serif; font-weight: 900; color: #cc8800; display: inline-block; }
            .numero-optimo-3d { font-size: 85px; font-family: 'Arial Black', sans-serif; font-weight: 900; color: #00994c; display: inline-block; }
            .card-semaforo-3d {
                background-color: #ffffff; padding: 20px; border-radius: 14px; border: 3px solid #0d1b2a;
            ...
            .card-semaforo-3d:hover { transform: translate(-2px, -2px); box-shadow: 9px 9px 0px #d4af37, 12px 12px 25px rgba(0,0,0,0.15) !important; }
        </style>
    ...
    # Desempaquetado dinámico de argumentos enviados desde app.py
    df_notas = None
    periodo_sel = None
    conn_sql = None
    ...
    # ⚡ EL MOTOR CONSOLIDADOR CORRECCIONAL (1 fila por estudiante)
    ...
    with c1:
        st.markdown(f"<div class='card-semaforo-3d' style='border-top:6px solid #cc0000;'><div class='titular-semaforo' style='color:#cc0000;'>🔴 Riesgo Crítico (&lt; 6.0)</div><div class='numero-critico-3d'>{count_criticos}</div></div>", unsafe_allow_html=True)
    with c2:
        st.markdown(f"<div class='card-semaforo-3d' style='border-top:6px solid #cc8800;'><div class='titular-semaforo' style='color:#cc8800;'>🟡 Alerta (6.0 - 7.5)</div><div class='numero-alerta-3d'>{count_alertas}</div></div>", unsafe_allow_html=True)
    with c3:
        st.markdown(f"<div class='card-semaforo-3d' style='border-top:6px solid #00994c;'><div class='titular-semaforo' style='color:#00994c;'>🟢 Óptimo (&gt;= 7.6)</div><div class='numero-optimo-3d'>{count_optimos}</div></div>", unsafe_allow_html=True)
    ...
    if not criticos.empty:
        ...
        st.success("✅ Perímetro estables: Cero estudiantes detectados en Riesgo Crítico.")
